...
const Navbar = () => {
  ...
  const socialIcons = [
    {
      link: "https://www.facebook.com/vincentxpatrick/",
      class: "hover:bg-blue-300",
      d: "M9 8h-3v4h3v12h5v-12h3.642l.358-4h-4v-1.667c0-.955.192-1.333 1.115-1.333h2.885v-5h-3.808c-3.596 0-5.192 1.583-5.192 4.615v3.385z",
      className: "h-7 w-7",
    },
    {
      link: "https://www.linkedin.com/in/vpmcastro/",
      class: "hover:bg-blue-300",
      d: "M4.98 3.5c0 1.381-1.11 2.5-2.48 2.5s-2.48-1.119-2.48-2.5c0-1.38 1.11-2.5 2.48-2.5s2.48 1.12 2.48 2.5zm.02 4.5h-5v16h5v-16zm7.982 0h-4.968v16h4.969v-8.399c0-4.67 6.029-5.052 6.029 0v8.399h4.988v-10.131c0-7.88-8.922-7.593-11.018-3.714v-2.155z",
      className: "h-7 w-7",
    },
    {
      link: "https://github.com/vixtroo",
      class: "hover:bg-blue-300",
    ...
    const handleScroll = () => {
      ...
      const sections = navLinks.map(({ title, link }) => {
        ...
        return {
          ...
          offsetBottom: targetElement
            ? targetElement.offsetTop + targetElement.offsetHeight - 50
            : 0,
        };
      });
      ...
      if (
        ...
        !document.activeElement.classList.contains("nav-link-clicked")
      ) {
  ...
  return (
    <div className="fixed z-10 flex flex-wrap content-center justify-between w-full h-20 px-10 mb-10 border-b border-blue-300 md:px-24 bg-slate-900">
      <div className="flex flex-wrap content-center justify-center h-full font-mono text-4xl font-semibold text-blue-300">
        <a href="#">Vixtro</a>
      </div>
      <div className="flex flex-wrap content-center justify-center h-full">
        <ul className="items-center hidden h-full gap-6 md:flex">
          {navLinks.map(({ title, link }) => {
            return (
              <li
                className={`relative block w-full h-full px-2 duration-100 delay-100 hover:border-b-2 ${
                  activeNav == title ? "border-b-2" : ""
                }`}
                key={title}
              >
                <a
      ...
      <div className="flex-wrap content-center justify-center hidden h-full gap-6 py-6 md:flex">
        <ul className="flex inline gap-x-2">
          {socialIcons.map((socialIcon, index) => (
            <li
              key={index}
              className={`flex flex-wrap content-center justify-center w-12 h-12 rounded-full bg-slate-500 delay-100 ${socialIcon.class} cursor-pointer duration-100`}
            >
              <Link
                href={socialIcon.link}
                className={`social-icons ${socialIcon.class} delay-100 duration-100`}
                target="blank"
              >
                <svg
      ...
      <Menu>
        <Menu.Button className="flex w-10 md:hidden">
          <FontAwesomeIcon
            icon={faBars}
